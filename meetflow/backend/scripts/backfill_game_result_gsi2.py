...
def _session_key(item: dict):
    """SESSION#{sessionNo}#RESULT#{userId} / SESSION#{sessionNo}#CHIP#{userId}
    から、親GameSessionアイテムのキー(PK, "SESSION#{sessionNo}")を組み立てる。
    """
    session_no = item["SK"].split("#")[1]
    return item["PK"], f"SESSION#{session_no}"


def _build_gsi2_attrs(item: dict, table, session_cache: dict):
    ...
    `gameType`がGameResult/GameResultChip自体には無い場合（DynamoDB物理
    設計書v1.13より前に登録されたデータ。GameSessionからの非正規化はv1.13で
    追加されたため）、親のGameSessionアイテムには常にgameTypeがあるため、
    そちらから補完する（同一セッション内の複数RESULT/CHIP行で重複して
    GetItemしないよう、session_cacheで使い回す）。
    """
    gsi1sk = item.get("GSI1SK", "")
    played_at = item.get("playedAt")
    if not gsi1sk.startswith("COMMUNITY#") or not played_at:
        return None
    community_id = gsi1sk.split("#", 2)[1]

    game_type = item.get("gameType")
    if not game_type:
        session_pk, session_sk = _session_key(item)
        cache_key = (session_pk, session_sk)
        if cache_key not in session_cache:
            session_item = table.get_item(
                Key={"PK": session_pk, "SK": session_sk}
            ).get("Item")
            session_cache[cache_key] = (session_item or {}).get("gameType")
        game_type = session_cache[cache_key]
    ...
    while True:
        ...
        for item in items:
            if not _is_game_result_or_chip(item) or not _needs_backfill(item):
                continue
            targets += 1
            attrs = _build_gsi2_attrs(item, table, session_cache)
            if attrs is None:
                skipped_incomplete += 1
                print(f"  [SKIP] gameTypeを特定できず算出不可: PK={item.get('PK')} SK={item.get('SK')}")
                continue
