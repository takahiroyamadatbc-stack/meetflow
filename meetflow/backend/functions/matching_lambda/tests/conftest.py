...
# （matching_lambda/）と共有Layer（layers/common/python）は両方とも
# sys.pathにマウントされる（それぞれ/var/taskと/opt/python） -- handler.py
# の`from handlers import conflict_detection, event_templates, matching`
# やmeetflow_common自身のimportを参照。
_MATCHING_LAMBDA_DIR = Path(__file__).resolve().parent.parent
_COMMON_LAYER_DIR = _MATCHING_LAMBDA_DIR.parent.parent / "layers" / "common" / "python"
for _path in (_MATCHING_LAMBDA_DIR, _COMMON_LAYER_DIR):
    if str(_path) not in sys.path:
        sys.path.insert(0, str(_path))

# 各ドメインLambdaはそれぞれ独自のトップレベル`handler`/`handlers`
# モジュールを持ち、各ドメインのtests/ディレクトリも同名の`_factories`
# ヘルパーモジュールを持つ。複数ドメインのテストを1つのpytestセッションで
# 実行すると、最初にimportされたドメインがsys.modulesにキャッシュされ、
# 他の全ドメインの同名モジュールを覆い隠してしまう。そのため各ドメインの
# conftestは、自分のテストモジュールがこれらをimportする前に退避させて
# おく必要がある。
