...
- 機能要件書v1.11
- DynamoDB物理設計書v1.19（特に単一テーブル設計のPK/SK/GSI1/GSI2）
...
- `backend/`：8ドメインLambda（User/Community/Availability/Matching/Event/Result/Notification/**Feedback**〔Issue #34で新規追加〕）+ 共通Layer（`meetflow_common`：DynamoDB/認証/ルーティング/EventBridge/エラーレスポンス等の共通処理）を実装済み。AvailabilityLambdaには要件定義書v1.3 §28（F-205/F-206、空き予定提出リクエスト）、NotificationLambdaには同§27（F-701〜F-704、プッシュ通知購読登録・解除＋実際のWeb Push送信）のハンドラーを追加済み。`AvailabilityRequestCreated`イベントはAvailabilityLambdaがpublishし、NotificationLambda側（`event_subscriber.py`→`push_sender.py`）が購読して通知レコード作成とWeb Push送信の両方を行うところまで配線済み。**FeedbackLambda**（`feedback_lambda/handlers/{feedback,attachments,announcements}.py`）はF-1401〜F-1406を実装済み：フィードバック投稿（1クリック評価/詳細投稿）・S3署名付きURLによるスクリーンショット添付・運営者限定の一覧/詳細/ステータス更新・返信・集計・アップデート予告CRUD。運営者判定（`cognito:groups`に`Operators`を含むか）はコミュニティのMembership.roleとは別軸のため、`meetflow_common`には置かずFeedbackLambda内に閉じた`handlers/operators.py`として実装（Lambda設計書v1.7 §9b.4の方針通り）。フィードバック返信時は`FeedbackReplied`イベントをEventBridge発行し、NotificationLambda（`event_subscriber.py`）が購読して投稿者へアプリ内通知を作成する。**pytest+moto(mock_aws)による単体テストは8ドメイン全て整備済み**（`functions/{domain}_lambda/tests/`、計306件）。メンバー自主退会（`POST /communities/{communityId}/members/me/leave`、F-104d）と、それを流用した強制退会（`update_member`のremove分岐）の未来の確定参加チェックは、`meetflow_common.participation.has_upcoming_reserved_participation`に共通ロジックとして実装済み（Issue #25/#26。Participant自体はcommunityIdを持たないためGSI1でユーザーの参加行を取得後、該当EventのGSI1PKからコミュニティを判定する）。UserLambdaのみ`handlers/`パッケージを持たない単一ファイル構成（`handler.py`が直接ロジックを実装）で、テストは`import handler`して`handler._get_profile(...)`のように呼ぶ。
  ...
  - **コミュニティ内ランキング（Issue #40、F-805/F-806）も実装済み**：ResultLambdaに`GET /communities/{communityId}/rankings`を追加し、コミュニティ×種目×期間の全メンバー分のGameResult/GameResultChipをGSI2（`GSI2PK=COMMUNITY#{communityId}#{gameType}`, `GSI2SK={playedAt}`）への1回のQueryで取得してLambda側で集計する（メンバー一覧を起点にしないため退会済みメンバーの記録も自動的に含まれる）。CommunityLambdaには`PUT /communities/{communityId}/ranking-settings`を追加し、ランキングのデフォルト表示条件（種目・集計期間・指標・足切り件数）をOWNER/ADMINが設定できるようにした。
...
- **DynamoDB**：単一テーブル（`MeetFlowTable`）、オンデマンドキャパシティ、PITR有効、KMS暗号化。GSIは2本のみで、オーバーロードGSIパターンを採用：
  ...
  - 各エンティティはPK/SKのプレフィックス（`USER#`、`COMMUNITY#`、`INVITE#`、`EVENT#`、`PLACE#`、`LOG#`等）でテーブルを共有する。全エンティティ・キー一覧は`docs/MeetFlow_DynamoDB物理設計書_v1.19.md` §3、集約されたアクセスパターン一覧は§4を参照。新しいクエリを追加する前に必ず読むこと。このスキーマ全体は場当たり的なクエリではなく、事前に洗い出されたアクセスパターン集合の上に構築されている。
