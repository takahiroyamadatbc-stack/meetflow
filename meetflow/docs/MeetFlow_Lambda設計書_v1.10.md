...
> **[MVP方針確認、v1.10で例外を追記]** 空き予定登録後にEventBridgeへイベントを自動発行する処理は原則として**入れない**。前回決定「F-401マッチング実行はMVPでは管理者による手動実行のみ」のため、登録時の自動マッチングトリガーは行わない -- この原則自体は変更していない。
>
...
>
> **[v1.10追加、Issue #97]** 上記のMVP方針に対する、狭く限定された2件目の例外として、空き予定の登録・バッチ登録・編集の都度`AvailabilityUpdated`イベントをEventBridgeへ発行する（9.1参照）。**F-401マッチング候補生成を自動実行するためのトリガーではない**点が上記`AvailabilityRequestCreated`と同じ位置付け -- 唯一の購読者はEventLambda（8.7参照）で、確定済み・定員未充足イベントとの時間帯・ゲーム種別の合致を判定し、合致した場合のみ別イベント（`ConfirmedEventCandidateAvailable`）を発行して管理者に知らせる。マッチングロジック自体（候補生成・スコアリング）を呼び出すことは一切ない。
...
- **`events:PutEvents`（`AvailabilityRequestCreated`発行用）[v1.2追加]**（**[v1.10追加]** `AvailabilityUpdated`発行にも同じ権限をそのまま使う。追加のgrantは不要）
...
| **EventBridge（`AvailabilityUpdated`購読）[v1.10新規]** | **確定済み・定員未充足イベントへの追加候補検知（7.6参照）** |
...
### 7.6 確定済みイベントへの追加候補検知 **[v1.10新規、Issue #97]**

```
AvailabilityLambdaが`AvailabilityUpdated`を発行（5.2参照。空き予定の登録/バッチ登録/編集の都度）
  ↓
EventLambdaが購読。当該コミュニティのEvent一覧をGSI1でQuery（GSI1PK=COMMUNITY#{id}, GSI1SK begins_with EVENT#）
  ↓
status=CONFIRMED、かつ時間帯が重複するEventだけに絞る
  ↓
templateIdを持たないEvent（Issue #56の手動作成候補由来）は判断材料が無いため対象外
  ↓
EventTemplateのgameTypeと一致するか判定。maxPlayersに対し現在のParticipant数（RESERVED_PARTICIPANT_STATUSES）が未満か判定
  ↓
対象ユーザーが既に当該Eventの参加者(CONFIRMED/AWAITING_APPROVAL)でないか確認
  ↓（全て通過）
EventBridgeへ`ConfirmedEventCandidateAvailable`を発行 → NotificationLambdaが該当コミュニティの管理者へ通知（9.2参照）
```

> **HITL厳守**：本フローは検知して管理者に知らせるだけで、参加者の追加は行わない。追加は既存の`POST /events/{eventId}/participants`（管理者操作、9.1参照）を管理者が通知から辿って実行する想定。「確定後フェーズ（min〜max）のボーナス席」という位置付けのため、確定前の候補生成（F-401）や仮確定フローとは独立している。
...
| **`ConfirmedEventCandidateAvailable`** [v1.10新規、Issue #97] | EventLambdaが7.6の判定で追加候補を検知した時 | NotificationLambda（該当コミュニティの管理者へ通知） |
...
- `events:PutEvents`（EventBridgeへの発行権限。`EventAwaitingApproval`/`EventParticipantRejected`用途を追加 [v1.6]。**`ConfirmedEventCandidateAvailable`発行用途を追加 [v1.10]**）
- **`AvailabilityUpdated`購読のためのEventBridge Rule + Lambda呼び出し権限（7.1/7.6参照）。判定処理自体はEvent/EventTemplate/Participantへの既存の`GetItem`/`Query`権限のみで完結し、新規IAM権限は不要 [v1.10追加]**
...
- **[v1.9追加]** GSI2へのQueryはIAM変更不要。`infra/meetflow_infra/meetflow_compute_stack.py`の`self.table.grant(fn, ...)`（CDK `ITable.grant`）は、テーブル本体と全インデックス（`{tableArn}/index/*`）の両方をまとめて許可リソースに含める実装のため、既存の`dynamodb:Query`権限がGSI1と同様GSI2にもそのまま及ぶ（MatchingLambda・FeedbackLambdaがGSI2を使う際も同じ理由で個別のインデックス指定なしに動いている、6章・9b章参照）。
...
| トリガー | 用途 |
|---|---|
| EventBridge | 各ドメインLambdaが発行するイベント（`EventConfirmed`, `EventCancelled`, `CancelApproved`, `CandidateConflictDetected`[v1.1追加], `AvailabilityRequestCreated`[v1.2追加], **`EventAwaitingApproval`/`EventParticipantRejected`[v1.6追加]**, **`ConfirmedEventCandidateAvailable`[v1.10追加、Issue #97]**等）を受けて通知レコードを作成 |
...
| メソッド/パス | 対応機能 |
|---|---|
| （EventBridgeイベント受信） | F-701 通知レコード作成（アプリ内通知 + Webプッシュ送信、9.3b参照）。**[v1.6追加]** `EventAwaitingApproval`は`detail.awaitingUserIds`（自動承認OFFで承認待ちのメンバーのみ、`participantIds`全員ではない）に承認依頼通知、`EventParticipantRejected`は該当コミュニティの管理者（OWNER/ADMIN）に辞退通知を送る。全員承認完了による本確定は、発行タイミングが仮確定時から遅くなるだけで意味は同じであるため、既存の`EventConfirmed`/確定通知メッセージをそのまま再利用する（新規メッセージは追加しない）。**[v1.10追加、Issue #97]** `ConfirmedEventCandidateAvailable`は`CandidateConflictDetected`と同じパターンで該当コミュニティの管理者（OWNER/ADMIN）にのみ通知し、`relatedEventId`に対象イベントを設定する（通知一覧タップで既存の`GET /events/{eventId}`詳細画面へ遷移し、既存の参加者追加操作へ導く） |
...
| 3 | 8.4 IAM最小権限に、GSI2へのQueryは既存の`self.table.grant`（CDK `ITable.grant`がテーブル本体+全インデックスをまとめて許可する実装のため）でカバー済みでIAM変更不要である旨を明記 | 上記と同一の決定事項 |
...
| No | 変更内容 | 対応する決定事項 |
|---|---|---|
| 1 | 5.2の「MVP方針確認」注記（空き予定登録後にEventBridgeイベントを自動発行しない）に、Issue #97専用の狭い例外を追記。F-401マッチング候補生成の自動実行トリガーにはしないという原則自体は維持したまま、空き予定の登録/バッチ登録/編集の都度`AvailabilityUpdated`イベントを発行するようにした | Issue #97：確定済みで定員に空きのあるイベントについて、別メンバーの空き予定更新で条件を満たしたら管理者に追加候補として通知する |
| 2 | 7.1 EventLambdaのトリガーに`AvailabilityUpdated`購読を追加。新規7.6節「確定済みイベントへの追加候補検知」で判定フローを定義（GSI1でコミュニティのCONFIRMEDイベントを取得→時間帯・ゲーム種別・定員未充足を判定→`ConfirmedEventCandidateAvailable`発行） | 上記と同一の決定事項 |
| 3 | 7.3のEventBridge発行イベント一覧に`ConfirmedEventCandidateAvailable`を追加。7.4のIAM最小権限に、判定処理は既存のEvent/EventTemplate/Participant向けgrantのみで完結し新規IAM権限が不要である旨を明記 | 上記と同一の決定事項 |
