# MeetFlow DynamoDB 物理テーブル設計書 v1.23

> 要件定義書v1.1・機能要件書v1.1・API設計書v1.1・操作ログ設計書v1.0・AWSシステム構成設計書v1.0を踏まえて設計。
> v1.0→v1.1、v1.1→v1.2、v1.2→v1.3、v1.3→v1.4、v1.4→v1.5、…、v1.22→v1.23の変更点はそれぞれ本文中と末尾の変更点サマリを参照。
...
| name | S | コミュニティ名 |
| description | S | 説明 |
| genre | S | ジャンル。固定選択肢6種（`麻雀`/`ポーカー`/`ボードゲーム`/`TRPG`/`スポーツ`/`その他`）のいずれか、必須（1コミュニティ=1ジャンル） **[v1.23修正]** |
...
>
> **[v1.23修正]** `genre`をIssue #92で自由入力から固定選択肢6種へ変更した。ResultLambdaの成績・ランキング機能（ウマ・オカ・トビ精算、`_RANKING_GAME_TYPES`等）が麻雀専用ロジックのため、ジャンルを識別可能な固定値にする必要があった。1コミュニティ＝1ジャンル（単一選択）とし、複数ジャンルの共存（案B）は別Issueで将来検討する。値自体を文字列として持つだけでキー構造への影響はなく、選択肢を増やす際は`communities.py`の`_ALLOWED_GENRES`配列に足すだけでよい。既存の（テスト・開発用）コミュニティの自由テキストgenre値からのバックフィルは、本番データがまだ存在しないため対応不要。
...
| createdAt | S | ISO8601 |
| memo | S | （任意）参加者向けのひとことメモ（持ち物・集合時間の補足・注意事項等）。会場に関する`locationNote`とは別物で、ステータスを問わずOWNER/ADMINがいつでも編集できる **[v1.21追加]** |
...
| createdAt | S | ISO8601 |
| ttl | N | DynamoDB TTL属性（テーブル側は既に`time_to_live_attribute="ttl"`で有効化済み、これまで実際に設定していたのはOperationLog(3.15)のみ）。作成時点でepoch秒 = createdAt + 90日を仮設定し、`mark_read`実行時にepoch秒 = 既読にした時刻 + 24時間へ上書きする **[v1.22追加]** |
...
>
> **[v1.22追加]** `ttl`はIssue #91対応。既読になった通知は24時間後、未読のままでも90日経過すれば自動削除の対象になる。DynamoDBのTTLによる実削除は失効後最大48時間程度遅延することがあるが、フロント表示自体は`read`フラグ＋作成日時で制御されているためこれは許容する（あくまでバックグラウンドのクリーンアップ）。
...
1b. **GameResult/GameResultChipへのGSI2属性バックフィル** [v1.19, v1.20で解消]：デプロイ済み環境に既に対局データが存在する場合、`GSI2PK`/`GSI2SK`未設定の既存アイテムはランキング集計（アクセスパターン36）から漏れる。`backend/scripts/backfill_game_result_gsi2.py`（GSI1SK・gameType・playedAtからGSI2属性を逆算してUpdateItemする一度限りのスクリプト、DEPLOY.md §3bに手順あり）を用意した。デプロイ環境ごとに既存データの有無を確認し、必要であれば`--dry-run`で内容確認後に実行する運用とする（未決事項としては解消。実行自体はデプロイ担当者の手動判断に委ねる）
...
## 28. v1.19 → v1.20 変更点サマリ

| No | 変更内容 | 理由 |
|---|---|---|
| 1 | 8章未決事項1bを解消。`backend/scripts/backfill_game_result_gsi2.py`（一度限りのバックフィルスクリプト）を用意し、DEPLOY.md §3bに実行手順を追記 | Issue #40のデプロイ作業で、dev環境に既存の対局データが存在する可能性が判明したため、検討事項のままにせず具体的な解決策を用意した |

Issue #40「コミュニティ内ランキング表示機能の追加」のデプロイ対応に伴う追補。

---

## 29. v1.20 → v1.21 変更点サマリ

| No | 変更内容 | 理由 |
|---|---|---|
| 1 | 3.10 Eventに`memo`（S、任意）を追加 | Issue #87：参加者向けのひとことメモ（持ち物・集合時間の補足・注意事項等）欄の追加。既存の`locationNote`（会場自由記述）とは別物で、ステータスを問わずOWNER/ADMINがいつでも編集できる |

---

## 30. v1.21 → v1.22 変更点サマリ

| No | 変更内容 | 理由 |
|---|---|---|
| 1 | 3.14 Notificationに`ttl`（N、任意）を追加 | Issue #91：通知に保持期限（TTL）を導入し、既読後24時間・未読90日で自動削除する。テーブル側は既にTTL属性（`ttl`）が有効化済みで、これまで実際に設定していたのはOperationLog(3.15)のみだったため、同じ仕組みをNotificationにも適用した |

---

## 31. v1.22 → v1.23 変更点サマリ

| No | 変更内容 | 理由 |
|---|---|---|
| 1 | 3.2 Communityの`genre`を自由入力から固定選択肢6種（麻雀/ポーカー/ボードゲーム/TRPG/スポーツ/その他）へ変更し、必須項目化 | Issue #92：コミュニティのジャンルを固定選択制にし、麻雀以外は成績・ランキング機能を非表示にする。Phase2として温存していた「麻雀以外の種目対応」の前倒し |
