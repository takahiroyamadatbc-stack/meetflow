...
**アクセスパターン**
- マッチング処理時「コミュニティ内の指定期間の空き予定を横断取得」：`PK=COMMUNITY#{id}, SK between AVAIL#{from} and AVAIL#{to}`
- ユーザー自身の空き予定一覧：GSI1
...
| 6 | コミュニティ内・期間指定の空き予定横断取得（マッチング用） | PK=COMMUNITY#{id}, SK between AVAIL#{from}~{to} |
