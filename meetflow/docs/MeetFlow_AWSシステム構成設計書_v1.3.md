...
用途：Reactアプリケーション配信。保存物：HTML, CSS, JavaScript/TypeScriptビルド成果物、**Service Workerスクリプト・Webアプリマニフェスト（PWA、[v1.3追加]）**

### CloudFront

用途：CDN配信。役割：HTTPS対応、キャッシュ、高速配信

> **[v1.3追加]** PWA化（要件定義書v1.3 27章）はReactアプリにService Workerとマニフェストファイルを追加するだけであり、S3/CloudFront構成自体への変更は不要。Service Workerのキャッシュ戦略上、CloudFrontのキャッシュ無効化パターンとの整合は実装段階で確認する（DynamoDB物理設計書v1.4 8章 未決事項2と同様の論点）。
...
**空き予定提出リクエストフロー** **[v1.3新規]**

```
AvailabilityLambda: POST /communities/{id}/availability-requests（リクエスト作成）
    ↓
EventBridge: AvailabilityRequestCreated 発行
    ↓
NotificationLambda（対象メンバーへ通知作成 + Webプッシュ送信）
```

> **[v1.3新規]** 要件定義書v1.3 28章に対応。マッチング実行とは無関係の通知専用フローであり、上記「マッチング自動起動には使わない」という方針とは矛盾しない（Lambda設計書v1.2 5.2参照）。
...
> **[v1.1修正]** v1.0では「MVP：メール通知（SES）」となっていたが、要件定義書v1.1 16章・機能要件書v1.1 12章では「MVP：アプリ内通知のみ」と定義されており、ドキュメント間で矛盾していた。Lambda設計書v1.0 9.3での指摘を受け、要件定義書・機能要件書を正とし、本書を修正していた。

**MVP**：アプリ内通知（NotificationLambdaがDynamoDBにレコード作成、フロントエンドが`GET /notifications`で取得）+ **Webプッシュ通知（PWA、[v1.3追加]）**

> **[v1.3修正]** 要件定義書v1.3 27章の通り、Push通知をMVP対象に前倒しする。ただし下記Phase2の「Amazon SNS or Firebase Cloud Messaging」という配信基盤は採用せず、**VAPID鍵によるWeb Push APIの直接呼び出し**とする（Lambda設計書v1.2 9.3b参照）。追加のAWSマネージドサービスは不要で、既存のNotificationLambda・EventBridge構成に処理を追加するのみ。VAPID鍵はAWS Secrets Managerに保管する（13章参照）。
...
AWS Secrets Manager 用途：外部APIキー、DB認証情報、**Webプッシュ通知のVAPID鍵ペア（公開鍵・秘密鍵）[v1.3追加]**

> **[v1.3追加]** NotificationLambdaが通知送信時にVAPID秘密鍵をSecrets Managerから取得する（Lambda設計書v1.2 9.3b/9.4参照）。鍵の生成・ローテーション方式は実装段階で決定する。
...
## 16. MVP時AWSサービス一覧 **[v1.3修正]**

| サービス | 用途 |
|---|---|
| CloudFront | 配信（PWAのService Worker・マニフェストを含む、[v1.3追加]） |
| S3 | フロントホスティング |
| Route53 | DNS |
| ACM | SSL |
...
| CloudWatch | 監視 |
| CloudTrail | 監査 |
| Secrets Manager | **Webプッシュ通知のVAPID鍵ペア保管（[v1.3追加]。以前から13章に記載はあったが本表への記載漏れだった外部APIキー等の用途も含む）** |
...
| No | 変更内容 | 理由 |
|---|---|---|
| 1 | 3章にPWA（Service Worker・マニフェスト）配信をS3/CloudFrontの用途として追加 | 要件定義書v1.3 27章：PWA化とWebプッシュ通知 |
| 2 | 9章のEventBridgeフロー図に空き予定提出リクエストフロー（`AvailabilityRequestCreated`）を追加 | 要件定義書v1.3 28章：管理者からの空き予定提出リクエスト |
