...
>
> **[v1.29修正]** `genre`は必須項目とし、固定選択肢6種（`麻雀`/`ポーカー`/`ボードゲーム`/`TRPG`/`スポーツ`/`その他`）のいずれかのみを受け付ける（Issue #92）。1コミュニティ＝1ジャンル（単一選択）。以前は自由入力の任意項目だったが、ResultLambdaの成績・ランキング機能が麻雀専用ロジック（ウマ・オカ・トビ精算等）に依存しており、ジャンルを識別可能な値に固定する必要があったため変更した。既存の（テスト・開発用）コミュニティの自由テキストgenre値からのバックフィルは、本番データがまだ存在しないため対応不要。選択肢はいつでも配列に追加するだけで増やせる設計（`community_lambda/handlers/communities.py`の`_ALLOWED_GENRES`）。
...
{
  "windows": [
    { "startTime": "2026-08-01T19:00:00Z", "endTime": "2026-08-01T22:00:00Z", "neededCount": 1 }
  ]
}
```

> **[v1.30新規]** Issue #96対応。CandidateMemberは6.1の候補生成を管理者が手動実行した後にしか存在しないため使えず、全メンバーの提出済みAvailabilityを起点に6.1と同じオーバーラップ計算（マッチング処理内部の交差区間ロジック）をその場で走らせ、`minPlayers`未満の窓を集計するAPI。
>
> **相互非公開型マッチングの原則（要件定義書v1.9 §19/§29.4）を維持するため、このコミュニティで（今日以降の日時の）空き予定を1件も提出していない呼び出し元には常に`windows: []`が返る**（後出しの傍観者に「どこに滑り込めば決定打になれるか」を教えないための唯一の防波堤）。この関門さえ通れば、呼び出し元が含まれていない窓（＝自分が1枠足せば成立させられる窓）も含めて全件返す -- 一度自分の空きを提出したメンバーは既に自分の手の内を晒しており、以降は駆け引きの対象にならないため。
>
> レスポンスは`neededCount`（あと何人必要か）という集計値のみで、誰が空きを出しているか（メンバーのuserId・nickname等）は一切含めない。「席確保」を約束するものではなく、成立への後押しに留める。
...
> **[v1.29追加]** `gameType`はバックエンド側では引き続き任意の文字列を受け付ける（新規のフォーマットバリデーションは追加しない）が、フロントエンドはコミュニティのジャンル（4.1参照）に応じて選択肢を出し分ける（Issue #92）：ジャンルが`麻雀`のコミュニティでは`MAHJONG4`/`MAHJONG3`から選択、それ以外のジャンルのコミュニティではコミュニティのジャンルと同じ固定値1つのみを使用し細分類は設けない（例：ジャンルが`ポーカー`のコミュニティでは`gameType`も常に`"ポーカー"`）。
...
権限はOWNER/ADMIN限定。イベントのステータスを問わずいつでも呼び出せる。現時点では参加者向けのひとことメモ（`memo`、持ち物・集合時間の補足・注意事項等。会場自由記述の`locationNote`とは別物）のみが更新対象。

Request

```json
{
  "memo": "現地集合、車の方は近隣コインパーキングをご利用ください"
}
```
...
### 8.4c イベント差し戻し（完了→開催予定） **[v1.28新規]**

`POST /events/{eventId}/reopen`

管理者（OWNER/ADMIN）が誤って「本日の対局を終了する」（8.4b）を実行してしまった場合の取り消し用。`COMPLETED`→`CONFIRMED`のみを許可する逆方向遷移で、他の状態からは呼び出せない。

処理：
- `COMPLETED`以外の状態からの呼び出しは`409 INVALID_STATUS_TRANSITION`
- 状態変更（`CONFIRMED`）、`EventStatusHistory`記録、OperationLog保存
- `EventStatusChanged`イベント発行（EventBridge）
- 登録済みの成績（GameResult）は削除・ブロックせずそのまま残す（`CONFIRMED`/`IN_PROGRESS`の間も成績編集自体は引き続き可能な設計のため、不整合にならない）
...
| No | 変更内容 | 対応する決定事項 |
|---|---|---|
| 1 | 8.2b イベント更新（`PUT /events/{eventId}`）を新規追加。参加者向けのひとことメモ（`memo`）をステータスを問わずOWNER/ADMINがいつでも編集できる | Issue #87：イベントに参加者向けの「ひとことメモ」欄を追加する |
...
| No | 変更内容 | 対応する決定事項 |
|---|---|---|
| 1 | 8.4c イベント差し戻し（`POST /events/{eventId}/reopen`）を新規追加。`COMPLETED`→`CONFIRMED`のみを許可する逆方向遷移（新規エラーコードなし、既存の`INVALID_STATUS_TRANSITION`を流用） | Issue #89：完了したイベントを開催予定（CONFIRMED）に戻せるようにする（管理者の操作ミス取り消し用） |
...
| No | 変更内容 | 対応する決定事項 |
|---|---|---|
| 1 | 4.1 コミュニティ作成の`genre`を必須項目化し、固定選択肢6種（麻雀/ポーカー/ボードゲーム/TRPG/スポーツ/その他）のみを受け付けるよう修正 | Issue #92：コミュニティのジャンルを固定選択制にし、麻雀以外は成績・ランキング機能を非表示にする（1コミュニティ＝1ジャンル） |
| 2 | 7.1 開催条件作成の`gameType`に、コミュニティのジャンルに応じてフロントエンドが選択肢を出し分ける旨を追記（バックエンド側のバリデーションは変更なし） | 上記と同一の決定事項 |
...
| No | 変更内容 | 対応する決定事項 |
|---|---|---|
| 1 | 6.4 「あと〇人で成立」集計取得（`GET /communities/{communityId}/matching/near-miss`）を新規追加。呼び出し元がこのコミュニティに空き予定を1件も提出していない場合は常に空配列を返し、相互非公開型マッチングの原則（要件定義書v1.9 §29.4）を維持する | Issue #96：空き予定を提出済みのメンバーに「あと〇人で成立」を見せ、カレンダーから該当枠を空き予定にする導線を出す |
